...
public class BackupCommand extends BaseCommand {
            ...
            if (file.exists()) {
                if (file.getName().endsWith(".yml")) {
                    ...
                    if (sender instanceof Player) {
                        ...
                        Object typed = pluginPlayer.getMetadata().get("backup_command");
                        if (typed != null) {
                            if (typed instanceof String) {
                                ...
                                if (typedFile.equals(file.getName())) {
                                    ...
                                    pluginPlayer.getMetadata().remove("backup_command");
                                } else {
                                    backupLoadConfirm(pluginPlayer, sender, file);
                                }
                            } else {
                                backupLoadConfirm(pluginPlayer, sender, file);
                            }
    ...
    private void backupLoadConfirm(PluginPlayer pluginPlayer, CommandSender sender, File file) {
        ...
        pluginPlayer.getMetadata().put("backup_command", file.getName());
        new BukkitRunnable() {
            @Override
            public void run() {
                pluginPlayer.getMetadata().remove("backup_command");
